...
=====================================
P2-M2  |  RecordEncoder — turns validated RawRecords into the exact raw
tensors TstccEncoder.forward() expects. Does NOT own nn.Embedding tables
or Time2Vec — those already exist, trained, inside the loaded TstccEncoder
...
------------------------------------------------------------------
    ...
    lengths_tensor            : (batch,)               int64, true length
                                 ...
                                 sequence support — see TstccEncoder._run_gru)
...
exact seq_len — RecordEncoder takes the most recent `seq_len` events per
...
class RecordEncoder:
    ...
    def encode_entity_records(
        ...
        records:
            ...
            (timestamp, metric_name) — same stable tiebreaker training
    ...
    def encode_event_batch(
        ...
        Per-entity tensors are NOT padded to a common length here — use
    ...
    def collate_encoded_batch(
        ...
        (entity_ids, numeric_tensor, categorical_tensor, timestamps_tensor, lengths_tensor)
            ...
            categorical_tensor : (batch, max_seq_len, 4)  — pad value 0 (PAD/UNK)
            timestamps_tensor  : (batch, max_seq_len, 1)
            lengths_tensor     : (batch,) int64 — true length per entity,
